...
def run_link_prediction(args, model, dataset, epoch, logger, is_test=False):
    ...
    for triplet in random.sample(evaluate_data, evaluate_size):
        ...
        if cnt_sample % 500 == 0:
            ...
            print_eval_info(acc_head, epoch, cnt_sample, hit10_head, hit3_head,
                            logger, rank_head, rec_rank_head)

    _hit10_head = print_eval_info(acc_head, epoch, evaluate_size, hit10_head, hit3_head, logger,
                    rank_head, rec_rank_head)

    return _hit10_head, hit10_head / cnt_sample


def print_eval_info(acc_head, epoch, evaluate_size, hit10_head, hit3_head, logger,
                    rank_head,  rec_rank_head):
